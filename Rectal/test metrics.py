...
diceq=[]
jaccardq=[]
hausdorff_distance_95q=[]
avg_surface_distanceq=[]


for i in range(1,409):
    gtpathori="E:\conse\labelsTs"
    predpathori="E:\conse\inferTs"
    gtpath = os.path.join(gtpathori,'colon_%04d'%i+'.nii.gz')
    predpath = os.path.join(predpathori,'colon_%04d'%i+'.nii.gz')
    a=sitk.ReadImage(gtpath)
    b=sitk.ReadImage(predpath)
    c = sitk.GetArrayFromImage(a)
    d= sitk.GetArrayFromImage(b)
    diceq.append(dice(d,c))
    jaccardq.append(jaccard(d,c))
    hausdorff_distance_95q.append(hausdorff_distance_95(d,c))
    avg_surface_distanceq.append(avg_surface_distance(d,c))

print(diceq)
print(jaccardq)
print(hausdorff_distance_95q)
print(avg_surface_distanceq)


mean1 = np.mean(diceq)
max1 = np.max(diceq)
min1 = np.min(diceq)
median1=np.median(diceq)
std1=np.std(diceq)

mean2 = np.mean(jaccardq)
max2 = np.max(jaccardq)
min2 = np.min(jaccardq)
median2=np.median(jaccardq)
std2=np.std(jaccardq)

mean3 = np.mean(hausdorff_distance_95q)
max3 = np.max(hausdorff_distance_95q)
min3 = np.min(hausdorff_distance_95q)
median3=np.median(hausdorff_distance_95q)
std3=np.std(hausdorff_distance_95q)

mean4 = np.mean(avg_surface_distanceq)
max4 = np.max(avg_surface_distanceq)
min4 = np.min(avg_surface_distanceq)
median4=np.median(avg_surface_distanceq)
std4=np.std(avg_surface_distanceq)
...
print(CI(mean1,std1,n),CI(mean2,std2,n),CI(mean3,std3,n),CI(mean4,std4,n))
